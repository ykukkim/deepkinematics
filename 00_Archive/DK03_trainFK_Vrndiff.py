...
from DK00_Utils.DK00_UT01_tranforms import CorrectIMUOrientation, ExtractInitialIMUOffset, ExtractWindow, TransformOrientation,ToTensor, \
...
def main(config=None):
    ...
    train_data = RealDataset(transform=train_transform, sampling_freq= C.SAMPLING_FREQUENCY,
                             version=config.VERSION, train=True)

    val_unseen_subjects_easy = RealDataset(test_set='easy', transform=unseen_subjects_transform, sampling_freq= C.SAMPLING_FREQUENCY,
                                           version=config.VERSION,
                                           train=False)

    val_unseen_subjects_hard = RealDataset(test_set='hard', transform=unseen_subjects_transform, sampling_freq= C.SAMPLING_FREQUENCY,
                                           version=config.VERSION,
                                           train=False)

    val_unseen_motion = RealDataset(test_set='unseen', transform=unseen_motion_transform, sampling_freq= C.SAMPLING_FREQUENCY,
                                    version=config.VERSION,
                                    train=False)
    ...
    for epoch in range(config.n_epochs):
        for i, abatch in enumerate(train_loader):
            ...
            if (epoch + 1) % config.eval_every == 0 and i == 0:
                ...
                easy_motion_losses, easy_motion_metrics = H.evaluate_and_log(dataset_name='easy', loader=unseen_subjects_easy_loader, model=model, vrn=vrn_net,
                ...
                hard_motion_losses, hard_motion_metrics = H.evaluate_and_log(dataset_name='hard', loader=unseen_subjects_hard_loader, model=model, vrn=vrn_net,
                                   preprocess_fn=preprocess_fn, metrics_evaluator=me,
                                   wandb_run=wandb_run, experiment_id=experiment_id,
                                   epoch=epoch+1,  VERSION=config.VERSION, modeltype=config.m_type)
                ...
                current_eval_metric_joint_rot = hard_motion_metrics['MPJAE [deg]']
