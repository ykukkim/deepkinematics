        ...
        for walk = 1:length(walks)
            try
                ...
                for param = 1:length(gait_params)
                    ...
                    % Subplot 1: Bland-Altman Plot for original root
                    %                     subplot(2,1,1);
                    BAP_or = BlandAltmanPlot(GT_root,predict_root);
                    %                     title([part ': ' walks{walk} ' - Original Root Added - ' gait_params{param}], 'Interpreter', 'none');
                    title([part ': ' walks{walk} ' - ' gait_params{param}], 'Interpreter', 'none');

                    %                     % Subplot 2: Bland-Altman Plot without any added root
                    %                     subplot(2,1,2);
                    %                     BAP = BlandAltmanPlot(GT_woroot,predict_woroot);
                    %                     title([part ': ' walks{walk} ' - Without Root added - ' gait_params{param}], 'Interpreter', 'none');
                    %
