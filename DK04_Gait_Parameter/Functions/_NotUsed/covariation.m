function CV_final=covariation(structure, Name, exercise, state, variable)
% calculate CV

run = strcmp(exercise, 'Run');
squat = strcmp(exercise, 'Squat');
...
if and(run, pre)
    Spatial=structure.(genvarname(Name)).(genvarname(exercise)).(genvarname(state)).Total.GaitParameters;
