function [HSlocs, TOlocs,zfootVel] = fva_DL_withroot(zheel, ztoe, srate, varargin)
...
zheel = filtfilt(b, a, detrend(zheel));
ztoe  = filtfilt(b, a, detrend(ztoe));
...
temp = Singular_spectrum_analysis(zfootVel,srate);
zfootVel = temp(:,1);
%% Determine TO events_Original

% mpd = floor(toprdfac * srate);
[TOpks, TOlocs] = findpeaks(zfootVel, 'minpeakdistance', 0.8*srate,'MinPeakHeight',-0.1);
TOlocs = TOlocs(TOpks > 0.05);         % x = TOlocs
...
zheel_temp = Singular_spectrum_analysis(zheel,srate);
[HSpks, HSlocs] = findpeaks(-zheel, 'MinPeakDistance',0.8*srate,'MinPeakHeight',-0.1); % the velocity values are multiplied by -1
