...
if __name__ == "__main__":
    ...
    for subject in [75]:#range(0,76):
        ...
        for file in config_v.trials:
            ...
            relative_phase = compute_relative_angle_for_skeleton(bvh._load_local_rotations())

            # === First figure: relative phase of left ankle ===
            fig1 = plt.figure()
            plt.plot(relative_phase[:, 3, 0],label='left', color='blue')  # First plot
            plt.plot(relative_phase[:, 3, 1],label='left', color='red')  # First plot
            plt.plot(relative_phase[:, 3, 2],label='left', color='green')  # First plot
