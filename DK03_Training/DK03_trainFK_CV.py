"""Train and aggregate participant-stratified cross-validation folds."""
...
def main(config=None):
    """Train every configured fold and write the combined CV summary."""
    ...
    if config.use_cross_validation:
        ...
        cv_results = {'easy': [], 'hard': [], 'unseen': []}
        cv_results_per_fold = []

        cv_save_dir = os.path.join(C.save_model_DIR, config.experimentid, "cross_validation")
    ...
    for fold_idx, (train_subj, val_subj) in enumerate(cv_folds):
        ...
        if config.predict_joints == True:
            experiment_name = f"{model.model_name()}-{'joint angles'}-{experiment_name}"
        else:
        ...
        checkpoint_file = os.path.join(model_dir, f"test_model-fold{fold_idx}.pth")
        ...
        for epoch in range(config.n_epochs):
            for i, abatch in enumerate(train_loader):
                ...
                if vrn_output is not None:
                    loss_dict['velocity_hat'] = vrn_net.backward(batch_gpu, vrn_output, config.m_type)
                    value = loss_dict.pop('total_loss', None)
                    if value is not None: # Add the key back to the dictionary at the end
                        loss_dict['total_loss'] = value
                    loss_values = {k: v.cpu().item() for k, v in loss_dict.items()}
                ...
                if (epoch + 1) % config.eval_every == 0 and i == 0:
                    ...
                    unseen_motion_losses, unseen_motion_metrics = H.evaluate_and_log(dataset_name='unseen', loader=unseen_motion_loader,
        ...
        metric_easy = H.final_evaluation_and_log(dataset_name='EASY', loader=unseen_subjects_easy_loader, model=model,
                                                 vrn=vrn_net,
                                                 preprocess_fn=preprocess_fn, metrics_evaluator=me, wandb_run=wandb_run,
                                                 experiment_id=experiment_id, VERSION=config.VERSION,
                                                 modeltype=config.m_type)

        metric_hard = H.final_evaluation_and_log(dataset_name='HARD', loader=unseen_subjects_hard_loader, model=model,
                                                 vrn=vrn_net,
                                                 preprocess_fn=preprocess_fn, metrics_evaluator=me, wandb_run=wandb_run,
                                                 experiment_id=experiment_id, VERSION=config.VERSION,
                                                 modeltype=config.m_type)

        metric_unseen = H.final_evaluation_and_log(dataset_name='UNSEEN', loader=unseen_motion_loader, model=model,
                                                   vrn=vrn_net,
                                                   preprocess_fn=preprocess_fn, metrics_evaluator=me,
                                                   wandb_run=wandb_run,
                                                   experiment_id=experiment_id, VERSION=config.VERSION,
    ...
    save_path = os.path.join(cv_save_dir, 'cv_results.xlsx')
