#!/usr/bin/env bash
# Parallel RNN training example; paths are resolved from this checkout.
set -euo pipefail
...
run_script "FK_Full_Adam_reduce" 'rnn' --VERSION 'FK' --bs_train 64 --n_epochs 200 --eval_every 10 --optimizer 'adam' --scheduler 'reduce' --lr 1e-3 --weight_decay 1e-6 --m_dropout 0.3 --m_pose_loss 10 --m_joint_rot_loss 20 --m_num_layers_RNN 2  --m_hidden_units_RNN '[[1024]]' --m_bidirectional --m_learn_init_state --use_acc_gyro --use_batch_norm --predict_arms --predict_head --predict_contact --predict_spl
