#!/usr/bin/env bash
#SBATCH -A ls_hilli
#SBATCH -n 4
#SBATCH --time=72:00:00
#SBATCH --mem-per-cpu=4000
#SBATCH --gpus=1
#SBATCH --gres=gpumem:24G
#SBATCH --output=$HOME/output_Training_FK_FULL_Adam_reduce_%j.txt

# Parallel velocity-plus-RNN training example.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
PYTHON_BIN="${PYTHON_BIN:-python}"
LOG_DIR="${DEEPKINEMATICS_LOG_DIR:-${PROJECT_ROOT}/logs}"
mkdir -p "${LOG_DIR}"

pids=""
index=0

# Function to run a python script and log the job ID

run_script() {
    local experiment_id=$1
    local m_type=$2
    shift 2

    # Capture all additional arguments as an array to preserve quoting
    local additional_args=("$@")
    local timestamp=$(date +%s)
    index=$((index + 1))
    local log_file="${LOG_DIR}/output_Training_${experiment_id}_${m_type}_${timestamp}_${index}.txt"

    # Start the python script in the background, redirecting both stdout and stderr to the same log file
    "${PYTHON_BIN}" "${PROJECT_ROOT}/DK03_Training/DK03_trainFK.py" \
        --experimentid "${experiment_id}" \
        --m_type "${m_type}" \
        "${additional_args[@]}" > "${log_file}" 2>&1 &

    local pid=$!
    echo "Script with experiment ID ${experiment_id} started with PID ${pid}"
    echo "${experiment_id}: ${pid}" >> "job_ids_${experiment_id}_${timestamp}".log
    pids="${pids} ${pid}"
}

# Running scripts simultaneously with different parameters
run_script "FK_Full_Adam_reduce" 'vrnrnn' --VERSION 'FK' --bs_train 64 --n_epochs 200 --eval_every 10 --optimizer 'adam' --scheduler 'reduce' --lr 1e-3 --weight_decay 1e-6 --m_dropout 0.3 --m_pose_loss 10 --m_joint_rot_loss 100 --m_num_layers_VRN 2 --m_hidden_units_VRN '[[1024]]' --m_embedding_VRNMLP '[[32]]' --m_num_layers_RNN 2 --m_hidden_units_RNN '[[64],[256]]' --m_bidirectional --m_learn_init_state --use_acc_gyro --use_batch_norm --predict_arms --predict_head --predict_contact --predict_spl
run_script "FK_Full_Adam_reduce" 'vrnrnn' --VERSION 'FK' --bs_train 64 --n_epochs 200 --eval_every 10 --optimizer 'adam' --scheduler 'reduce' --lr 1e-3 --weight_decay 1e-6 --m_dropout 0.3 --m_pose_loss 10 --m_joint_rot_loss 100 --m_num_layers_VRN 2 --m_hidden_units_VRN '[[256]]' --m_embedding_VRNMLP '[[32]]' --m_num_layers_RNN 2  --m_hidden_units_RNN '[[512],[1024]]' --m_bidirectional --m_learn_init_state --use_acc_gyro --use_batch_norm --predict_arms --predict_head --predict_contact --predict_spl

for pid in ${pids}; do
    wait $pid
done

echo "All Python scripts completed."
