"""Create the combined spatiotemporal gait-parameter comparison figure."""
...
output_dir = "plots"
filename = "spatiotemporal_group_comparison_coloured_GT_lines.pdf"
...
gt_colors = ['black', 'darkred', 'darkgreen']
width = 0.2
...
for i, param in enumerate(parameters):
    ...
    # Ground truth dashed lines per group in different colours
    for xi in range(len(groups)):
        ax.hlines(gt_values[i, xi], xi - 0.4, xi + 0.4, colors=gt_colors[xi], linestyles='dashed', linewidth=1.5)

    ax.set_title(param.replace("_", " "), fontsize=14)
    ax.set_xticks(x)
...
fig.legend(handles, labels + ['GT All', 'GT Young', 'GT Old'],
           loc='lower center', ncol=6, frameon=False, fontsize=12)
